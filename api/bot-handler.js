...
export async function onUpdate(data, botApi, Reactions, RestrictedChats, botUsername, RandomLevel) {
    ...
    if (data.message || data.channel_post) {
        ...
        // 3️⃣ --- ميزة معرفة الايدي (ID) والمعلومات ---
        ...
        // 4️⃣ --- أمر فحص الحالة (خاص بك فقط) ---
        else if (data.message && text === '/status' && content.from.id === 5794792675) { // 🔴 استبدل الرقم بالايدي مالتك
            const date = new Date().toLocaleString('ar-IQ', { timeZone: 'Asia/Baghdad' });
            await botApi.sendMessage(chatId, `✅ البوت متصل وشغال!\n⏰ الوقت: ${date}\n📍 الموقع: الناصرية`);
        }

        // 5️⃣ --- ردود ذكية تلقائية ---
            ...
            await botApi.sendMessage(chatId, "✅ التفاعلات المفعمة: \n\n" + reactions);
        } 
